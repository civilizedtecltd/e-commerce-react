...
const logout = () => (dispatch) => {
	...
	setAuthToken();
	axios
		.post(URL._LOGOUT, { refreshToken: jwt.refreshToken })
		.then((res) => {
...
const confirmOrder = (data) => (dispatch) => {
	setAuthToken();
	axios
		.post(URL._CONFIRM_ORDER, { ...data })
		.then((res) => {
			if (res.data & res.data.mail)
				localStorage.setItem("usr_mail", JSON.stringify(res.data.mail));
			dispatch({
...
const updatePaymentMethod = (data) => (dispatch) => {
	setAuthToken();
	axios
		.post(URL.UPDATE_PAYMENT_METHOD, data)
		.then((res) => {
			return dispatch({
